...
def execute_dump_motion(ctx: MotionContext):
    ...
    # 1) 버리는 위치로 이동
    dump_x, dump_y, dump_z, dump_rx, dump_ry, dump_rz = DUMP_POSITION
    dump_pos = posx([dump_x, dump_y, dump_z, current_pos[3], current_pos[4], current_pos[5]])
    ...
    # 2) Z값을 오프셋만큼 내림
    lower_z = dump_z + Z_OFFSET
    lower_pos = posx([dump_x, dump_y, lower_z, current_pos[3], current_pos[4], current_pos[5]])
    
    ctx.node.get_logger().info(f'내려가는 중 (Z={lower_z:.2f})...')
    ctx.motion.movel(
        lower_pos,
        vel=[100.0, 150.0],
        acc=[100.0, 100.0],
